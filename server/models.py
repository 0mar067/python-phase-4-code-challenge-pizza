...
class Restaurant(db.Model, SerializerMixin):
    # BEGINNER COMMENT: This model represents a restaurant that can serve multiple pizzas
    ...
    # BEGINNER COMMENT: One-to-many relationship with RestaurantPizza
    ...
    # BEGINNER COMMENT: Serialization rules to control what data is included when converting to dict
...
class Pizza(db.Model, SerializerMixin):
    # BEGINNER COMMENT: This model represents a pizza that can be served at multiple restaurants
    ...
    # BEGINNER COMMENT: Simple serialization - just include basic pizza info
...
class RestaurantPizza(db.Model, SerializerMixin):
    # BEGINNER COMMENT: This model represents the link between restaurants and pizzas
    ...
    # BEGINNER COMMENT: Foreign key to restaurant table
    restaurant_id = db.Column(db.Integer, db.ForeignKey("restaurants.id"))
    # BEGINNER COMMENT: Foreign key to pizza table
    pizza_id = db.Column(db.Integer, db.ForeignKey("pizzas.id"))

    # BEGINNER COMMENT: Relationships to both parent models
    restaurant = db.relationship("Restaurant", back_populates="restaurant_pizzas")
    pizza = db.relationship("Pizza", back_populates="restaurant_pizzas")

    # BEGINNER COMMENT: Serialization rules for RestaurantPizza
    # We include the full pizza and restaurant objects (not just IDs)
    # This is what the POST response wants
    serialize_rules = ("-restaurant.restaurant_pizzas", "-pizza.restaurant_pizzas")

    # BEGINNER COMMENT: Validation method to ensure price is between 1 and 30 inclusive
    @validates("price")
    def validate_price(self, key, price):
        # BEGINNER COMMENT: Check if price is provided
        if price is None:
            raise ValueError("Price is required")
        # BEGINNER COMMENT: Check if price is within valid range
        if not (1 <= price <= 30):
